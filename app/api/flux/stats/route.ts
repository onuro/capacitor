  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries - 1) {
        console.log(
          `[Stats] Retry ${attempt + 1}/${maxRetries} for ${url} after ${retryDelay}ms`,
        );
...
function transformDockerStats(
  rawStats: DockerStats,
  ...
  const diskUsage =
    rawStats.disk_stats?.status !== "error"
  ...
  try {
    ...
    if (specData.status === "success" && specData.data) {
      if (specData.data.compose) {
        componentNames = specData.data.compose.map(
          (c: { name: string; hdd: number }) => {
  ...
  for (let nodeIndex = 0; nodeIndex < ips.length; nodeIndex++) {
    ...
    for (const containerName of containerNames) {
      try {
        ...
        console.log(
          `Trying node: ${nodeIp} -> ${nodeUrl} (max retries: ${maxRetries})`,
        );
        ...
        if (!response.ok || !contentType?.includes("application/json")) {
          ...
          console.log(
            `[Stats] ${nodeIp} returned ${response.status}: ${text.slice(0, 100)}`,
          );
        ...
        console.error(
          `Error fetching stats from ${nodeIp}/${containerName}:`,
          error,
        );
      }
    }
